                        ...
                        <div class="form-group col-md-6">
                            ...
                            <div class="">
                                <select class="custom-select form-control" v-model="resource.parent_id" id="parent_id">
                                    ...
                                    <option v-for="option in sortedParents" v-bind:value="option.id">
                                        {{ option.title }}
                                    </option>
                                </select>
                            </div>
        ...
        methods: {
            goTime() {
                ...
                vm.$http.get(vm.appResourceUrl + '/create').then(function(response) {
                    if ( response.data && response.data.parents && response.data.parents.length )
            ...
            listenEvents() {
                ...
                vm.$on('successfulcreate', function(data) {
                    ...
                    if ( data.response )
                        vm.$router.push({name: 'admin_developer_guides.view', params: { id: data.response.data.id }});
                });
            }
        },
        watch: {
