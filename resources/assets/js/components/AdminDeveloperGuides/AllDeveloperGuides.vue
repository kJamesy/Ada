<template>
    <div class="mt-3">
                    ...
                    <table class="table table-bordered table-hover table-info">
                        <thead>
                            <tr class="pointer-cursor">
                                ...
                                <th v-on:click.prevent="appChangeSort('order')">Menu Order <span v-html="appGetSortMarkup('order')"></span></th>
                            ...
                            <tr v-for="resource in orderedAppResources">
                                ...
                                <td v-if="appUserHasPermission('read')">
                                    <router-link v-bind:to="{ name: 'admin_developer_guides.view', params: { id: resource.id }}" class="btn btn-sm rounded-circle btn-pink"><i class="icon ion-eye"></i></router-link>
                                </td>
                            </tr>
                        </tbody>
                    </table>
        ...
        <div class="mt-3 mb-3 font-italic text-right" v-if="! fetchingData && appDeletedNum">
            <router-link v-bind:to="{ name: 'admin_developer_guides.trash'}" class="btn btn-link"><i class="icon ion-trash-a"></i> Deleted Items ({{ appDeletedNum }})</router-link>
        </div>
    </div>
</template>
