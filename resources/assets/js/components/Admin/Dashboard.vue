<template>
    <div class="mt-5">
        ...
        <div v-if="! fetchingData">
            <p class="mb-3">Welcome back, {{ profile.name }}</p>

            <div class="row" v-if="lastEmail.injections_count">
                <div class="col-sm-6">
                    <div class="card">
                        <div class="card-body">
                            <h4 class="card-title">{{ lastEmail.subject }}</h4>
                            <p class="card-text">
                                Sent: <span v-bind:title="lastEmail.sent_at | dateToTheMinWithDayOfWeek" data-toggle="tooltip">{{ lastEmail.sent_at | dateToTheDay }}</span> <br />
                                Recipients: {{ lastEmail.injections_count }}
                            </p>
            ...
            <div class="row mt-4">
                <div class="col-sm-6" v-if="nextScheduledEmail.sent_at && nextScheduledEmail.recipients_num">
                    <div class="card">
                        <div class="card-body">
                            <h4 class="card-title">{{ nextScheduledEmail.subject }}</h4>
                            <p class="card-text">
                                Scheduled For: <span v-bind:title="nextScheduledEmail.sent_at | dateToTheMinWithDayOfWeek" data-toggle="tooltip">{{ nextScheduledEmail.sent_at | dateToTheDay }}</span> <br />
                                Recipients: {{ nextScheduledEmail.recipients_num }}
                            </p>
                        </div>
                    </div>
                </div>
                <div class="col-sm-6">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <p class="card-text">
                                New Today: {{ todaysSubscribers.length }} <br />
